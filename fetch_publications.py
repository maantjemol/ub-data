    ...
    try:
        ...
        for record in root.xpath("//oai:record", namespaces=namespaces):
            ...
            identifier = record.xpath(
                "./oai:header/oai:identifier/text()", namespaces=namespaces
            )
            if identifier:
                record_data["identifier"] = identifier[0]

            datestamp = record.xpath(
                "./oai:header/oai:datestamp/text()", namespaces=namespaces
            )
            if datestamp:
        ...
        resumption_tokens = root.xpath(
            "//oai:resumptionToken/text()", namespaces=namespaces
        )
        token = (
            resumption_tokens[0]
            if resumption_tokens and resumption_tokens[0].strip()
            else None
        )
        ...
        try:
            async with session.get(BASE_URL, params=params, timeout=30) as response:
                if response.status == 503:  # Service Unavailable
                    ...
                    print(
                        f"Server busy (503). Waiting {wait_time}s before retry {i+1}/{retry_count}"
                    )
            ...
            print(
                f"Request timed out. Waiting {wait_time}s before retry {i+1}/{retry_count}"
            )
            await asyncio.sleep(wait_time)
        except Exception as e:
            if i < retry_count - 1:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while resumption_tokens:
            ...
            while len(tasks) < max_concurrent and resumption_tokens:
                ...
                token_params = {"verb": "ListRecords", "resumptionToken": token}
